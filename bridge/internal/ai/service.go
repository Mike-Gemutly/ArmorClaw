...
func (s *AIService) getClient(provider ProviderType, apiKey, baseURL string) (AIClient, error) {
	...
	family, err := providerFamily(provider, baseURL)
	if err != nil {
		return nil, &AIError{
	...
	var newClient AIClient

	switch family {
	case ProviderFamilyOpenAICompatible:
		newClient = NewOpenAIClient(apiKey, baseURL)
	case ProviderFamilyAnthropic:
		newClient = NewAnthropicClient(apiKey, baseURL)
	default:
		return nil, &AIError{
			Code:      "unsupported_provider",
			Message:   fmt.Sprintf("provider %q is registered but not supported by bridge ai service", provider),
			Retryable: false,
		}
	}

	s.clients[provider] = newClient
