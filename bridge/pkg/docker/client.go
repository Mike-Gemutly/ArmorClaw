...
// ============================================================
// Pre-warm Pool — Reduces cold start latency by maintaining
// a pool of pre-created (but not yet started) containers.
// ============================================================
...
func NewPrewarmPool(client *Client, config PrewarmPoolConfig) *PrewarmPool {
	if config.PoolSize <= 0 {
		config.PoolSize = 2
	}
	if config.RefillDelay <= 0 {
		config.RefillDelay = 5 * time.Second
	}

	return &PrewarmPool{
		client: client,
		config: config,
		pool:   make([]string, 0, config.PoolSize),
		stopCh: make(chan struct{}),
	}
}

// Acquire takes a pre-warmed container from the pool.
// Returns the container ID and true if available, or empty string and false.
// The caller is responsible for starting the container.
func (p *PrewarmPool) Acquire() (string, bool) {
	p.mu.Lock()
	defer p.mu.Unlock()

	if len(p.pool) == 0 {
		dockerTracker.Event("prewarm_miss", map[string]any{"pool_size": 0})
		return "", false
	}

	containerID := p.pool[0]
	p.pool = p.pool[1:]

	dockerTracker.Event("prewarm_acquire", map[string]any{
		"container_id": containerID[:min(12, len(containerID))],
...
func (p *PrewarmPool) Start(ctx context.Context) {
	p.mu.Lock()
	if p.running {
		p.mu.Unlock()
		return
	...
	dockerTracker.Event("prewarm_start", map[string]any{"pool_size": p.config.PoolSize})

	go p.refillLoop(ctx)
}

// Stop halts the background refill loop and drains the pool
func (p *PrewarmPool) Stop(ctx context.Context) {
	p.mu.Lock()
	p.running = false
	close(p.stopCh)

	// Drain pool: remove all pre-warmed containers
	for _, id := range p.pool {
		_ = p.client.RemoveContainer(ctx, id, true)
	}
	p.pool = nil
	p.mu.Unlock()

	dockerTracker.Event("prewarm_stop", nil)
}

// Size returns the current number of pre-warmed containers
func (p *PrewarmPool) Size() int {
	p.mu.Lock()
	defer p.mu.Unlock()
	return len(p.pool)
}

// refillLoop runs in the background, creating containers to maintain pool size
func (p *PrewarmPool) refillLoop(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		...
		case <-time.After(p.config.RefillDelay):
			p.mu.Lock()
			deficit := p.config.PoolSize - len(p.pool)
			p.mu.Unlock()

			for i := 0; i < deficit; i++ {
				containerID, err := p.client.CreateContainer(
					ctx,
					p.config.Config,
					p.config.HostConfig,
					nil, nil,
				...
				if err != nil {
					dockerTracker.Failure("prewarm_create", err, map[string]any{"deficit": deficit})
					break // Back off on failure
				}

				p.mu.Lock()
				p.pool = append(p.pool, containerID)
				p.mu.Unlock()

				dockerTracker.Success("prewarm_create", map[string]any{
					"container_id": containerID[:min(12, len(containerID))],
					"pool_size":    p.Size(),
				})
			}
