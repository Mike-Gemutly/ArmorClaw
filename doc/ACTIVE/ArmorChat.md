...
> **ArmorChat Version:** 4.1.1-alpha03
> **Review Type:** Post-Fix Integration Verification + Frontend UX Review + ArmorClaw 0.3.4 Spec Alignment + ArmorClaw Compatibility Review
...
| G-09 | No migration path v2.5 → v4.1+ (no legacy data wipe) | §v2.5 → v4.1 upgrade screen | ✅ Fixed — Auto-detect + manual recovery + legacy wipe |
...
## 5. User Migration v2.5 → v4.1 (G-09)

### ArmorClaw Spec Requirement
ArmorClaw.md §G-09 (Migration Path) requires a v2.5 → v4.1 upgrade screen so users with legacy Bridge-only sessions can migrate to the new Matrix-native architecture.
...
Total routes after all fixes: **55** (was 37 before fixes)

New routes added (post-review):
- `MIGRATION` — v2.5 → v4.1 upgrade flow
...
- `TUTORIAL`, `ONBOARDING_CONFIG`, `ONBOARDING_SETUP`, `ONBOARDING_INVITE` — Onboarding deep links
- `INVITE`, `SERVER_CONNECTION` — Post-onboarding setup
- `AGENT_MANAGEMENT`, `HITL_APPROVALS`, `WORKFLOW_MANAGEMENT`, `BUDGET_STATUS` — Agent/workflow management
- `LICENSES`, `TERMS_OF_SERVICE` — Legal screens
- `USER_PROFILE`, `SHARED_ROOMS` — User profile screens
...
## 11. ArmorClaw 0.3.4 Spec Alignment (SA-01 through SA-06)
...
- See section 15 (RC-01 through RC-05) for additional required ArmorChat changes.
...
| File | Purpose |
|------|---------|
| `androidApp/.../screens/onboarding/MigrationScreen.kt` | v2.5→v4.1 migration flow + legacy data wipe |
...
| `androidApp/.../navigation/AppNavigation.kt` | 18 new routes (55 total), nav graph wiring |
...
## 15. ArmorClaw Compatibility Review — Required ArmorChat Changes (2026-02-26)

Cross-referencing ArmorChat code expectations against the actual ArmorClaw bridge codebase revealed 3 issues fixed bridge-side and 5 ArmorChat-side changes required for flawless operation.

### ArmorClaw Bridge Fixes Applied (This Review)

| # | Issue | Severity | Fix Applied |
|---|-------|----------|-------------|
| 1 | `bridge.health` RPC returned `status: "healthy"` — ArmorChat SA-01 checks `status == "ok"` | CRITICAL | Changed to `"ok"` in `handleBridgeHealth()` |
| 2 | `bridge.health` RPC lacked 0.3.4 fields (`bridge_ready`, `is_new_server`, `provisioning_available`, `server_name`) | CRITICAL | Added all fields; `is_new_server` reads from `provisioningMgr.IsOwnerClaimed()` |
| 3 | `health` RPC returned `status: "healthy"` instead of `"ok"` | HIGH | Aligned to `"ok"` in `handleHealth()` |

**Files Modified:**
- `bridge/pkg/rpc/server.go` — `handleBridgeHealth()` now returns all 0.3.4 fields + capability flags; `handleHealth()` returns `status: "ok"`
- `bridge/pkg/provisioning/manager.go` — Added `IsOwnerClaimed()` public accessor for `ownerClaimed` state

### Required ArmorChat Changes

The following changes are required or strongly recommended in ArmorChat to fully leverage the ArmorClaw bridge contract.

#### RC-01: Wire `admin_token` into RPC auth headers (HIGH)
**Current state:** SA-03 stores `admin_token` from `provisioning.claim` into `SetupCompleteInfo.adminToken`, but no subsequent RPC call attaches it.
**Required:** `BridgeRpcClient` must include `Authorization: Bearer <admin_token>` on all post-setup RPC calls. The bridge generates tokens with prefix `aat_` (ArmorClaw Admin Token). Without this, future auth-gated RPC methods will reject requests.
**Files:** `BridgeRpcClientImpl.kt` — add `Authorization` header to `httpPost()` using stored `adminToken`.

#### RC-02: Gate provisioning claim on `provisioning_available` (MEDIUM)
**Current state:** SA-02 extracts `provisioning_available` into `BridgeHealthDetails` but the claim flow doesn't check it.
**Required:** Before calling `provisioning.claim`, check `bridgeHealthDetails.provisioningAvailable`. If `false`, skip claim and fall back directly to `bridge.status` role check. This handles bridges running without a provisioning secret.
**Files:** `SetupService.kt` — add guard in `connectWithCredentials()` before the claim attempt.

#### RC-03: Consume `matrix_homeserver` from claim response (MEDIUM)
**Current state:** `ProvisioningClaimResponse` model has 6 fields (`success`, `role`, `admin_token`, `user_id`, `device_id`, `message`). The bridge also returns `matrix_homeserver` and `correlation_id`.
**Required:** Add `@SerialName("matrix_homeserver") matrixHomeserver: String?` and `@SerialName("correlation_id") correlationId: String?` to `ProvisioningClaimResponse`. When `matrixHomeserver` is present, use it to override the QR-derived homeserver URL — this handles cases where the bridge's configured homeserver differs from what was in the QR.
**Files:** `RpcModels.kt` (`ProvisioningClaimResponse`), `SetupService.kt` (homeserver override logic).

#### RC-04: Use push gateway URL from discovery instead of hardcoded default (MEDIUM)
**Current state:** `PushNotificationRepository` uses a hardcoded default `https://push.armorclaw.app/_matrix/push/v1/notify`.
**Required:** The push gateway URL should come from the QR payload (`push_gateway` field), the `/discover` endpoint, or the well-known response (`com.armorclaw.push_gateway`). For self-hosted deployments, Sygnal runs on a different host/port than the bridge — the hardcoded URL won't work.
**Files:** `PushNotificationRepository.kt` — read `push_gateway` from `SetupConfig`/`ServerConfig` instead of hardcoded constant.

#### RC-05: Consolidate `isIpAddress()` into shared utility (LOW)
**Current state:** SA-06 fixed the 4th copy of `isIpAddress()` to use strict IPv4 regex, but 4 independent copies still exist.
**Required:** Extract into a single `NetworkUtils.isIpAddress()` in the shared module. Replace all 4 call sites (`SetupService.kt`, `SetupViewModel.kt`, `ConnectServerScreen.kt`, `RpcModels.kt`).
**Files:** Create `shared/.../util/NetworkUtils.kt`, update all 4 consumers.

### Updated Spec Verification

| ArmorClaw Response Field | RPC Method | ArmorChat Consumer | Status |
|--------------------------|------------|-------------------|--------|
| `status: "ok"` | `bridge.health`, `health`, HTTP `/health` | `performBridgeHealthCheck()` | ✅ Aligned |
| `bridge_ready` | `bridge.health`, HTTP `/health` | `BridgeHealthStatus.NOT_READY` gating | ✅ Aligned |
| `provisioning_available` | `bridge.health`, HTTP `/health` | `BridgeHealthDetails` (stored, not yet gating claim) | ⚠️ RC-02 |
| `is_new_server` | `bridge.health`, HTTP `/health` | `DetectServerInfo.isNewServer` | ✅ Aligned |
| `server_name` | `bridge.health`, HTTP `/health` | Informational display | ✅ Aligned |
| `admin_token` (claim) | `provisioning.claim` | `SetupCompleteInfo.adminToken` (stored, not used in auth) | ⚠️ RC-01 |
| `matrix_homeserver` (claim) | `provisioning.claim` | Not consumed | ⚠️ RC-03 |
| `correlation_id` (claim) | `provisioning.claim` | Not consumed | ⚠️ RC-03 |
| `push_gateway` (QR/discover) | QR payload, `/discover`, well-known | Hardcoded default used instead | ⚠️ RC-04 |
| `supports_*` capability flags | `bridge.health` | Not yet consumed (future feature gating) | ℹ️ Optional |

---

*Last reviewed: 2026-02-26 (updated for ArmorClaw compatibility review + bridge RPC fixes)*
