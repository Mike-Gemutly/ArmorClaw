...
# API key stored in encrypted keystore

[keystore]
path = "${DATA_DIR}/keystore.db"
...
initialize_keystore() {
    ...
    print_info "Creating encrypted keystore..."

    # Generate keystore salt
    local salt=$(openssl rand -hex 16 2>/dev/null || cat /dev/urandom | tr -dc 'a-f0-9' | fold -w 32 | head -n 1)
    echo "$salt" > "$DATA_DIR/.keystore_salt"
    chmod 600 "$DATA_DIR/.keystore_salt"

    # Store API key (simplified - real implementation would use proper encryption)
    print_info "Storing API key in keystore..."

    # Create initial keystore entry
    mkdir -p "$DATA_DIR/secrets"
    echo "{\"api_key\":\"$API_KEY\",\"base_url\":\"$API_BASE_URL\"}" > "$DATA_DIR/secrets/api.json"
    chmod 600 "$DATA_DIR/secrets/api.json"
